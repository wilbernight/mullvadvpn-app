...
pub struct RetryFuture2<R: Future> {
    factory: Box<dyn FnMut() -> R>,
    should_retry: Box<dyn FnMut(&R::Output) -> bool>,
    delays: Box<dyn Iterator<Item = Duration>>,
}

impl<R: Future> RetryFuture2<R> {
    ...
    pub fn new(
        factory: impl FnMut() -> R + 'static,
        should_retry: impl FnMut(&R::Output) -> bool + 'static,
    ...
    pub fn set_should_retry(&mut self, should_retry: impl FnMut(&R::Output) -> bool + 'static) {
    ...
    pub async fn into_future(mut self) -> R::Output {
        loop {
