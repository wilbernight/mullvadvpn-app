...
pub trait FutureFactory {
    ...
    type Output: Future<Output = Self::Result>;
    /// Type that `Output` resolves to.
    type Result;
...
impl<F, O: Future<Output = R>, R> FutureFactory for F where F: FnMut() -> O,
{
    type Output = O;
    type Result = R;
    ...
    > RetryFuture<F, R, D>
{
    ...
    pub fn set_should_retry<NewR: FnMut(&F::Result) -> bool + 'static>(
        self,
        should_retry: NewR,
    ...
    pub async fn into_future(mut self) -> F::Result {
        loop {
